...
def test_each_line(solution: str, test_settings):
    ...
    solution_lines = [line.strip() for line in solution_lines if line and not line.startswith('%')]
...
def print_result(result: list[int] | None):
    ...
    print(f"N\t{fp}\t{tn}")
    print()
    print('Accuracy:\t', "{:.3f}".format((tp+tn)/count))
    print('Precision:\t', "{:.3f}".format(tp/(tp+fp)) if tp+fp > 0 else np.nan)
    print('Recall:\t\t', "{:.3f}".format(tp/(tp+fn)) if tp+fn > 0 else np.nan)


if __name__ == '__main__':
    parser = argparse.ArgumentParser('test')

    parser.add_argument('--dir', choices=['train', 'test'], default='test')
    parser.add_argument('--line', action='store_true', help='test each line in the solution file, does not work with recursion')
    ...
    print_result(result)
    if args.line:
        test_each_line(solution, {'exs_file': exs_file, 'bk_file': bk_file})
