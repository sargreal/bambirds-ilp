...
import random
from math import sqrt, ceil, atan2, cos, sin, pi, degrees, radians, tan
...
blocks = {'1':[1,1], '2':[1,0.5], '3':[0.5,1], '4':[0.5,0.5],
          '5':[0.25,0.25], '6':[0.5,0.25], '7':[0.25,0.5], '8':[1,0.25],
          '9':[0.25,1], '10':[1.5,0.25], '11':[0.25,1.5],
          '12':[2,0.25], '13':[0.25,2]}
...
def choose_item(probability_table, rng=random._inst):
    ran_num = rng.uniform(0.0,1.0)
    selected_num = 0
    while ran_num > 0:
...
def add_new_row(current_tree_bottom, total_tree, new_row_attempts, rng=random._inst):

    groupings = generate_subsets(current_tree_bottom)   # generate possible groupings of bottom row objects
    choosen_item = choose_item(probability_table_blocks, rng)# choosen block for new row
    ...
    if total_options > 0:
        option = rng.randint(1,total_options)
        ...
        return add_new_row(current_tree_bottom, total_tree, new_row_attempts+1, rng) # choose a new block and try again if no options available
...
def make_peaks(center_point,rng=random._inst):

    current_tree_bottom = []        # bottom blocks of structure
    number_peaks = rng.randint(1,max_peaks)     # this is the number of peaks the structure will have
    top_item = choose_item(probability_table_blocks,rng)    # this is the item at top of structure

    if number_peaks == 1:
        current_tree_bottom.append([top_item,center_point])     

    if number_peaks == 2:
        distance_apart_extra = round(rng.randint(min_peak_split,max_peak_split)/100.0,10)
...
def make_structure(absolute_ground, center_point, max_width, max_height, rng=random._inst) -> Tuple[list,list,list] :
    ...
    current_tree_bottom = make_peaks(center_point,rng)
    if max_width > 0.0:
    ...
    if max_height > 0.0 or max_width > 0.0:
        ...
        while structure_height < max_height and structure_width < max_width and row_add_successful:
            total_tree, current_tree_bottom, row_add_successful = add_new_row(current_tree_bottom, total_tree, new_row_attempts,rng)
            complete_locations = []
...
def create_ground_structures(number_ground_structures, rng=random._inst):
    valid = False
    current_attempt = 0
    while valid == False:
        ground_divides = []
        ...
        for i in range(number_ground_structures-1):
            ground_divides.insert(i+1,rng.uniform(level_width_min, level_width_max))
        valid = True
    ...
    for i in range(len(ground_positions)):
        ...
        complete_locations2, final_pig_positions2, pig_protect_values2 = make_structure(absolute_ground, center_point, max_width, max_height, rng)
...
def create_platforms(number_platforms, complete_locations, possible_pig_positions, rng=random._inst):

    platform_centers = []
    attempts = 0            # number of attempts so far to find space for platform
    final_platforms = []
    while len(final_platforms) < number_platforms:
        platform_width = rng.randint(4,7)
        platform_position = [rng.uniform(level_width_min+((platform_width*platform_size[0])/2.0), level_width_max-((platform_width*platform_size[0])/2.0)),
                             rng.uniform(level_height_min, (level_height_max - minimum_height_gap))]
        temp_platform = []
...
def create_platform_structures(final_platforms, platform_centers, complete_locations, final_pig_positions, pig_protect_values, rng=random._inst):
    current_platform = 0
    for platform_set in final_platforms:
...
def add_additional_blocks(possible_trihole_positions, possible_tri_positions, possible_cir_positions, possible_cirsmall_positions, rng=random._inst):
    all_other = []
    ...
    while (len(all_other) > 0):
        chosen = all_other.pop(rng.randint(0,len(all_other)-1))
...
def add_pigs(number_pigs, possible_pig_positions, complete_locations, pig_protect_values, final_platforms,extra_platforms,rng=random._inst):
    final_pig_positions = []
    ...
    while len(final_pig_positions) < number_pigs and num_attempts < max_attempts:
        ...
        # if no remaining options then place pigs randomly on the ground  
            ...
            test_position = [rng.uniform(level_width_min, level_width_max),absolute_ground]
            pig_width = pig_size[0]
            pig_height = pig_size[1]
            valid_pig = True
            for structure in complete_locations:
...
def protect_vulnerable_blocks1(complete_locations, complete_ground_locations, final_platforms, vulnerable_blocks, final_pig_positions, selected_other, rng=random._inst):
    ...
    for vul in vulnerable_blocks:
        for structure in complete_ground_locations:
            if vul in structure:
                ...
                buffer = rng.uniform(buffer_min,buffer_max)
                ...
                while (number_attempts < max_number_attempts):
                    ...
                    if (overlap == False):
                        ...
                        number_attempts = 0
                    overlap = False
                    choosen_item = choose_item(probability_table_blocks,rng)
                    if new_stack == []:
...
def write_level_xml(final_blocks, selected_other, final_pig_positions, final_platforms, number_birds, bird_order, final_materials, final_tnt_positions, extra_platforms_angled, current_level, rng=random._inst):
    ...
    for i in selected_other:
        material = materials[str(choose_item(probability_table_materials,rng))]       # material is chosen randomly
        if i[0] == '1':
...
def swap_blocks(complete_locations, final_pig_positions, final_platforms, rng=random._inst):
    if (block_swapping == True):
        ...
        for i in range(len(complete_locations)):
            for j in range(len(complete_locations[i])):
                ...
                for block in test_blocks:
                    if rng.uniform(0.0,1.0) < (probability_table_blocks[str(block[0])]/total_prob_amount):
...
def protect_vulnerable_blocks(complete_locations, complete_ground_locations, final_platforms, final_pig_positions, selected_other, rng=random._inst):
    vulnerable_blocks = []
    if (vulnerability_analysis == True):
        ...
        if (protection_method1 == True):
            complete_locations = protect_vulnerable_blocks1(complete_locations, complete_ground_locations, final_platforms, vulnerable_blocks, final_pig_positions, selected_other,rng)
...
def set_materials(complete_locations, final_pig_positions, vulnerable_blocks, rng=random._inst):
    final_materials = []
    final_blocks = []
    ...
    for grouping in blocks_in_way:
        if (rng.uniform(0.0,1.0) < trajectory_chance):
            material_choice = choose_item(probability_table_materials_trajectory, rng)
    ...
    for structure in complete_locations:
        ...
        if rng.uniform(0.0,1.0) < cluster_chance:
            all_set = 0
            current_point = rng.randint(0,len(structure)-1)
            #current_point = 0
            start_point = current_point
            material_choice = choose_item(probability_table_materials,rng)
            while (all_set == 0):
                ...
                for i in range(len(structure)):
                    if final_materials[index+i] == 0:
                        if sqrt( ((structure[i][1]-structure[start_point][1]) * (structure[i][1]-structure[start_point][1])) +
                            ...
                            current_point = i
                            if rng.uniform(0.0,1.0) < cluster_swap_prob:
                                material_choice = choose_item(probability_table_materials,rng)
                                start_point = current_point
        ...
        elif rng.uniform(0.0,1.0) < random_chance:
            for block in structure:
                material_choice = choose_item(probability_table_materials,rng)
...
def create_hills(complete_locations, possible_pig_positions,ground_divides, rng=random._inst):
    extra_platforms = []
    ...
    for i in range(len(complete_locations)):
        ...
        cur_increase = rng.uniform(0.0,max_slope_increase)-(max_slope_increase/2.0)
...
if __name__ == "__main__":
  ...
  while (finished_levels < number_levels):
      ...
      number_ground_structures, complete_locations, possible_pig_positions, pig_protect_values, ground_divides = create_ground_structures(number_ground_structures, random)
