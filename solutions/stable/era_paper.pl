...
stable(A) :- object(B), object(C), stable(B), stable(C), verticalContact(B,A), verticalContact(C,A),
  (
...
stable(A) :- object(B), stable(B), verticalContact(B,A),
  (
    once(lessStarts(A,B);
    mostFinishesI(A,B);
    lessFinishes(A,B);
...
stable(A) :- object(B), object(C), stable(B), stable(C), horizontalContact(A,B), horizontalContact(A,C), 
  (
    (
      meetsI(A,B),
      once(
...
stable(A) :-
  object(B), object(C), stable(B), stable(C), 
  verticalContact(B,A), verticalContact(C,A),
    (
      once(
        mostOverlapsMostI(A,B);
        mostOverlapsLessI(A,B);
...
stable(A) :-
  object(B), object(C), stable(B), stable(C), 
  horizontalContact(B,A), verticalContact(C,A),
  (
    (
      meetsI(A,B),
      once(
...
stable(A) :-
  object(B), object(C), stable(B), stable(C), 
  horizontalContact(B,A), horizontalContact(C,A),
  meetsI(A,B),
  meets(A,C), !.