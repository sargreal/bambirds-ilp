...
for file in $(ls ${ROOT_DIR}/solutions/${target}/intermediate); do
    ...
    # Skip if "filter" in the dataset name
    ...
    # generate the evaluation data
    infile=${ROOT_DIR}/solutions/${target}/intermediate/$file
    outdir=${ROOT_DIR}/evaluation/${target}/history
    python3 ${SCRIPT_DIR}/test.py --dir train --set ${target}_$dataset --output ${outdir}/train_$file history ${infile}
    python3 ${SCRIPT_DIR}/test.py --dir test --set ${target}_$dataset --output ${outdir}/test_$file history ${infile}
