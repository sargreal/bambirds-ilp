belongsTo(ice9,struct1).
belongsTo(pig0,struct3).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone10,struct2).
belongsTo(stone11,struct4).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct4,struct3).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice9,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone10,bar).
hasForm(stone11,bar).
hasForm(stone2,bar).
hasForm(stone3,block).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasMaterial(ice9,ice,453,305,9,51).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,355,347,25,6).
hasMaterial(stone1,stone,382,342,25,9).
hasMaterial(stone10,stone,485,245,25,5).
hasMaterial(stone11,stone,533,196,25,5).
hasMaterial(stone2,stone,389,336,24,10).
hasMaterial(stone3,stone,389,303,19,22).
hasMaterial(stone4,stone,390,350,24,6).
hasMaterial(stone5,stone,392,330,25,10).
hasMaterial(stone6,stone,396,324,25,10).
hasMaterial(stone7,stone,408,301,10,25).
hasMaterial(stone8,stone,421,328,6,25).
hasOrientation(ice9,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone10,horizontal).
hasOrientation(stone11,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,vertical).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,horizontal).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice9,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone10,medium).
hasSize(stone11,medium).
hasSize(stone2,small).
hasSize(stone3,medium).
hasSize(stone4,small).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice9,struct1).
isAnchorPointFor(pig0,struct3).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone10,struct2).
isAnchorPointFor(stone11,struct4).
isBelow(ice9,hill2).
isBelow(stone1,stone2).
isBelow(stone1,stone5).
isBelow(stone2,stone5).
isBelow(stone2,stone6).
isBelow(stone4,stone1).
isBelow(stone4,stone2).
isBelow(stone5,stone6).
isBelow(stone5,stone7).
isBelow(stone6,stone3).
isBelow(stone6,stone7).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(stone0,stone1).
isLeft(stone3,stone7).
isLeft(stone5,stone8).
isLeft(stone6,stone8).
isLeft(stone7,hill2).
isOn(ice9,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone1,hill0).
isOn(stone1,stone4).
isOn(stone10,hill2).
isOn(stone11,hill3).
isOn(stone2,hill0).
isOn(stone2,stone1).
isOn(stone2,stone4).
isOn(stone3,hill0).
isOn(stone3,stone6).
isOn(stone4,hill0).
isOn(stone5,hill0).
isOn(stone5,stone1).
isOn(stone5,stone2).
isOn(stone6,hill0).
isOn(stone6,stone2).
isOn(stone6,stone5).
isOn(stone7,hill0).
isOn(stone7,stone5).
isOn(stone7,stone6).
isOn(stone8,hill0).
isOver(hill0,pig0).
isOver(stone10,pig0).
isRight(stone1,stone0).
isRight(stone3,hill1).
isRight(stone7,stone3).
isRight(stone8,stone5).
isRight(stone8,stone6).
isTower(struct1).
pig(pig0,504,342,10,10).
protects(struct2,pig0).
scene_scale(81,0.99885551).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice9,rect, 457.5,330.5,339,[6.65102757,51.0376561,1.5393804]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 367.5,350,150,[6,25,0]).
shape(stone1,rect, 394.5,346.5,171,[6.86060606,25.01345873,0.06283185]).
shape(stone10,rect, 497.5,247.5,125,[5,25,0]).
shape(stone11,rect, 545.5,198.5,125,[5,25,0]).
shape(stone2,rect, 401,341,175,[7.21162921,24.2699204,0.09424778]).
shape(stone3,rect, 398.5,314,128,[5.17643662,24.83853545,0.87964594]).
shape(stone4,rect, 402,353,144,[6,24,0]).
shape(stone5,rect, 404.5,335,173,[6.94480291,25.053534,0.12566371]).
shape(stone6,rect, 408.5,329,175,[6.95535408,25.30420047,0.12566371]).
shape(stone7,rect, 413,313.5,149,[5.92613004,25.16151191,1.72787596]).
shape(stone8,rect, 424,340.5,150,[6,25,1.57079633]).
situation_name('situation1_0-8-12-5-1').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
