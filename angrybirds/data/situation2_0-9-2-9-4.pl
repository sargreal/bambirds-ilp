belongsTo(ice10,struct3).
belongsTo(ice11,struct3).
belongsTo(ice9,struct1).
belongsTo(pig0,struct2).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone12,struct3).
belongsTo(stone14,struct3).
belongsTo(stone15,struct3).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(wood13,struct3).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,bar).
hasForm(ice11,bar).
hasForm(ice9,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone12,bar).
hasForm(stone14,block).
hasForm(stone15,block).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(wood13,bar).
hasMaterial(ice10,ice,479,195,32,6).
hasMaterial(ice11,ice,483,201,13,51).
hasMaterial(ice9,ice,455,305,6,51).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,388,343,25,7).
hasMaterial(stone1,stone,389,349,24,7).
hasMaterial(stone12,stone,488,164,8,25).
hasMaterial(stone14,stone,510,172,22,17).
hasMaterial(stone15,stone,513,181,23,18).
hasMaterial(stone2,stone,389,337,25,7).
hasMaterial(stone3,stone,389,299,6,25).
hasMaterial(stone4,stone,390,331,25,6).
hasMaterial(stone5,stone,394,324,24,7).
hasMaterial(stone6,stone,397,300,6,24).
hasMaterial(stone7,stone,404,299,7,26).
hasMaterial(stone8,stone,411,300,7,25).
hasMaterial(wood13,wood,490,189,25,6).
hasOrientation(ice10,horizontal).
hasOrientation(ice11,vertical).
hasOrientation(ice9,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone12,vertical).
hasOrientation(stone14,horizontal).
hasOrientation(stone15,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,vertical).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,vertical).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(wood13,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,medium).
hasSize(ice11,big).
hasSize(ice9,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,small).
hasSize(stone12,medium).
hasSize(stone14,medium).
hasSize(stone15,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,small).
hasSize(stone6,small).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(wood13,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct3).
isAnchorPointFor(ice9,struct1).
isAnchorPointFor(pig0,struct2).
isAnchorPointFor(stone0,struct0).
isBelow(ice10,wood13).
isBelow(ice11,ice10).
isBelow(ice9,hill2).
isBelow(stone0,stone2).
isBelow(stone1,stone0).
isBelow(stone15,stone14).
isBelow(stone2,stone4).
isBelow(stone4,stone5).
isBelow(stone5,stone3).
isBelow(stone5,stone6).
isBelow(stone5,stone7).
isBelow(stone5,stone8).
isBelow(wood13,stone12).
isBelow(wood13,stone14).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isLeft(ice10,stone15).
isLeft(stone3,stone6).
isLeft(stone6,stone7).
isLeft(stone7,stone8).
isLeft(stone8,hill2).
isOn(ice10,ice11).
isOn(ice11,hill2).
isOn(ice9,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,stone1).
isOn(stone1,hill0).
isOn(stone12,wood13).
isOn(stone14,stone15).
isOn(stone14,wood13).
isOn(stone15,hill3).
isOn(stone2,hill0).
isOn(stone2,stone0).
isOn(stone3,hill0).
isOn(stone3,stone5).
isOn(stone4,hill0).
isOn(stone4,stone2).
isOn(stone5,hill0).
isOn(stone5,stone4).
isOn(stone6,hill0).
isOn(stone6,stone5).
isOn(stone7,hill0).
isOn(stone7,stone5).
isOn(stone8,hill0).
isOn(stone8,stone5).
isOn(wood13,ice10).
isOn(wood13,stone15).
isOver(hill0,pig0).
isOver(ice10,pig0).
isOver(stone14,pig0).
isOver(stone15,pig0).
isOver(wood13,pig0).
isRight(stone15,ice10).
isRight(stone15,wood13).
isRight(stone3,hill1).
isRight(stone6,stone3).
isRight(stone7,stone6).
isRight(stone8,stone7).
isTower(struct0).
isTower(struct1).
isTower(struct3).
pig(pig0,504,342,10,10).
protects(struct3,pig0).
scene_scale(81,0.99571437).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 495,198,192,[6,32,0]).
shape(ice11,rect, 489.5,226.5,321,[6.46457231,49.74161947,1.44513262]).
shape(ice9,rect, 458,330.5,306,[6,51,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 400.5,346.5,175,[7,25,0]).
shape(stone1,rect, 401,352.5,168,[7,24,0]).
shape(stone12,rect, 492,176.5,142,[5.68082936,25.13903977,1.50796447]).
shape(stone14,rect, 521,180.5,112,[4.62502296,24.37868979,0.5969026]).
shape(stone15,rect, 524.5,190,149,[6.3028568,23.67503814,0.53407075]).
shape(stone2,rect, 401.5,340.5,175,[7,25,0]).
shape(stone3,rect, 392,311.5,150,[6,25,1.57079633]).
shape(stone4,rect, 402.5,334,150,[6,25,0]).
shape(stone5,rect, 406,327.5,168,[7,24,0]).
shape(stone6,rect, 400,312,144,[6,24,1.57079633]).
shape(stone7,rect, 407.5,312,182,[7,26,1.57079633]).
shape(stone8,rect, 414.5,312.5,175,[7,25,1.57079633]).
shape(wood13,rect, 502.5,192,150,[6,25,0]).
situation_name('situation1_0-9-2-9-4').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
