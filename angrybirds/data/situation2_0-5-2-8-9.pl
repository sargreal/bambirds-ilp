belongsTo(ice10,struct1).
belongsTo(ice11,struct2).
belongsTo(ice12,struct2).
belongsTo(ice6,struct0).
belongsTo(pig0,struct3).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone13,struct4).
belongsTo(stone14,struct4).
belongsTo(stone15,struct4).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(stone9,struct0).
belongsTo(wood16,struct4).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct3,struct4).
canCollapse(struct4,struct3).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,bar).
hasForm(ice11,block).
hasForm(ice12,block).
hasForm(ice6,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone13,block).
hasForm(stone14,block).
hasForm(stone15,block).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(stone9,bar).
hasForm(wood16,bar).
hasMaterial(ice10,ice,453,305,5,51).
hasMaterial(ice11,ice,493,242,7,8).
hasMaterial(ice12,ice,496,221,19,27).
hasMaterial(ice6,ice,395,307,6,51).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,389,300,26,9).
hasMaterial(stone1,stone,389,252,7,25).
hasMaterial(stone13,stone,504,193,20,21).
hasMaterial(stone14,stone,508,179,23,18).
hasMaterial(stone15,stone,517,178,23,18).
hasMaterial(stone2,stone,391,294,24,9).
hasMaterial(stone3,stone,392,288,25,9).
hasMaterial(stone4,stone,392,282,25,9).
hasMaterial(stone5,stone,393,277,25,6).
hasMaterial(stone7,stone,398,252,6,25).
hasMaterial(stone8,stone,403,252,9,25).
hasMaterial(stone9,stone,411,253,7,24).
hasMaterial(wood16,wood,538,195,25,6).
hasOrientation(ice10,vertical).
hasOrientation(ice11,vertical).
hasOrientation(ice12,vertical).
hasOrientation(ice6,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,vertical).
hasOrientation(stone13,vertical).
hasOrientation(stone14,horizontal).
hasOrientation(stone15,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(stone9,vertical).
hasOrientation(wood16,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,big).
hasSize(ice11,medium).
hasSize(ice12,medium).
hasSize(ice6,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone13,medium).
hasSize(stone14,medium).
hasSize(stone15,medium).
hasSize(stone2,small).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(stone9,small).
hasSize(wood16,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct1).
isAnchorPointFor(ice11,struct2).
isAnchorPointFor(pig0,struct3).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone13,struct4).
isBelow(ice10,hill2).
isBelow(ice11,ice12).
isBelow(ice6,stone0).
isBelow(ice6,stone2).
isBelow(stone0,stone2).
isBelow(stone0,stone3).
isBelow(stone13,stone14).
isBelow(stone13,stone15).
isBelow(stone14,stone15).
isBelow(stone2,stone3).
isBelow(stone2,stone4).
isBelow(stone3,stone4).
isBelow(stone4,stone5).
isBelow(stone5,stone1).
isBelow(stone5,stone7).
isBelow(stone5,stone8).
isBelow(stone5,stone9).
isBelow(stone9,stone8).
isBelow(wood16,stone15).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(stone0,hill2).
isLeft(stone1,stone7).
isLeft(stone2,hill2).
isLeft(stone3,hill2).
isLeft(stone4,hill2).
isLeft(stone5,hill2).
isLeft(stone7,stone8).
isLeft(stone8,stone9).
isLeft(stone9,hill2).
isOn(ice10,hill0).
isOn(ice11,hill2).
isOn(ice12,hill2).
isOn(ice12,ice11).
isOn(ice6,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,ice6).
isOn(stone1,stone5).
isOn(stone13,hill3).
isOn(stone14,stone13).
isOn(stone15,stone13).
isOn(stone15,stone14).
isOn(stone15,wood16).
isOn(stone2,hill0).
isOn(stone2,ice6).
isOn(stone2,stone0).
isOn(stone3,hill0).
isOn(stone3,stone0).
isOn(stone3,stone2).
isOn(stone4,hill0).
isOn(stone4,stone2).
isOn(stone4,stone3).
isOn(stone5,stone4).
isOn(stone7,stone5).
isOn(stone8,stone5).
isOn(stone9,stone5).
isOn(wood16,hill3).
isOver(hill0,pig0).
isOver(ice11,pig0).
isOver(ice12,pig0).
isOver(stone13,pig0).
isOver(stone14,pig0).
isOver(stone15,pig0).
isRight(stone0,hill1).
isRight(stone1,hill1).
isRight(stone2,hill1).
isRight(stone3,hill1).
isRight(stone4,hill1).
isRight(stone7,stone1).
isRight(stone8,stone7).
isTower(struct0).
isTower(struct1).
isTower(struct2).
pig(pig0,504,342,10,10).
protects(struct2,pig0).
protects(struct4,pig0).
scene_scale(81,0.99650163).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 455.5,330.5,255,[5,51,1.57079633]).
shape(ice11,rect, 496.5,246,38,[4.72405494,8.04567932,1.75929189]).
shape(ice12,rect, 505.5,234.5,161,[5.59945557,28.8394903,2.10486708]).
shape(ice6,rect, 398,332.5,306,[6,51,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 402,304.5,157,[6.02785062,26.07282771,0.09424778]).
shape(stone1,rect, 392.5,264.5,175,[7,25,1.57079633]).
shape(stone13,rect, 514,203.5,102,[4.15170416,24.80315762,2.32477856]).
shape(stone14,rect, 519.5,188,117,[4.99942373,23.60031643,0.62831853]).
shape(stone15,rect, 528.5,187,115,[4.72957523,24.41077878,0.5969026]).
shape(stone2,rect, 403,298.5,164,[6.79781554,24.20380356,0.06283185]).
shape(stone3,rect, 404.5,292.5,169,[6.76586778,25.07726574,0.09424778]).
shape(stone4,rect, 404.5,286.5,174,[6.91445492,25.17137406,0.09424778]).
shape(stone5,rect, 405.5,280,150,[6,25,0]).
shape(stone7,rect, 401,264.5,150,[6,25,1.57079633]).
shape(stone8,rect, 407.5,264.5,172,[6.86060606,25.13903977,1.63362818]).
shape(stone9,rect, 414.5,265,168,[7,24,1.57079633]).
shape(wood16,rect, 550.5,198,150,[6,25,0]).
situation_name('situation1_0-5-2-8-9').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
