belongsTo(ice10,struct1).
belongsTo(ice12,struct3).
belongsTo(ice6,struct0).
belongsTo(pig0,struct2).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone11,struct3).
belongsTo(stone14,struct3).
belongsTo(stone15,struct3).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(stone9,struct0).
belongsTo(wood13,struct3).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,bar).
hasForm(ice12,bar).
hasForm(ice6,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone11,bar).
hasForm(stone14,bar).
hasForm(stone15,bar).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(stone9,bar).
hasForm(wood13,bar).
hasMaterial(ice10,ice,453,305,5,51).
hasMaterial(ice12,ice,490,202,16,49).
hasMaterial(ice6,ice,397,306,6,52).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,389,250,7,25).
hasMaterial(stone1,stone,390,301,25,6).
hasMaterial(stone11,stone,461,245,25,6).
hasMaterial(stone14,stone,529,189,25,6).
hasMaterial(stone15,stone,530,195,24,6).
hasMaterial(stone2,stone,390,294,24,7).
hasMaterial(stone3,stone,390,288,25,6).
hasMaterial(stone4,stone,390,280,25,9).
hasMaterial(stone5,stone,391,275,24,6).
hasMaterial(stone7,stone,398,250,6,25).
hasMaterial(stone8,stone,404,250,7,25).
hasMaterial(stone9,stone,411,250,7,25).
hasMaterial(wood13,wood,502,195,24,7).
hasOrientation(ice10,vertical).
hasOrientation(ice12,vertical).
hasOrientation(ice6,vertical).
hasOrientation(stone0,vertical).
hasOrientation(stone1,horizontal).
hasOrientation(stone11,horizontal).
hasOrientation(stone14,horizontal).
hasOrientation(stone15,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(stone9,vertical).
hasOrientation(wood13,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,big).
hasSize(ice12,big).
hasSize(ice6,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone11,medium).
hasSize(stone14,medium).
hasSize(stone15,small).
hasSize(stone2,small).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,small).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(stone9,medium).
hasSize(wood13,small).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct1).
isAnchorPointFor(pig0,struct2).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone11,struct3).
isBelow(ice10,hill2).
isBelow(ice12,wood13).
isBelow(ice6,stone1).
isBelow(stone1,stone2).
isBelow(stone15,stone14).
isBelow(stone2,stone3).
isBelow(stone3,stone4).
isBelow(stone4,stone5).
isBelow(stone5,stone0).
isBelow(stone5,stone7).
isBelow(stone5,stone8).
isBelow(stone5,stone9).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isLeft(stone0,stone7).
isLeft(stone1,hill2).
isLeft(stone11,ice12).
isLeft(stone2,hill2).
isLeft(stone3,hill2).
isLeft(stone4,hill2).
isLeft(stone5,hill2).
isLeft(stone7,stone8).
isLeft(stone8,stone9).
isLeft(stone9,hill2).
isLeft(wood13,stone15).
isOn(ice10,hill0).
isOn(ice12,hill2).
isOn(ice6,hill0).
isOn(pig0,hill0).
isOn(stone0,stone5).
isOn(stone1,hill0).
isOn(stone1,ice6).
isOn(stone11,hill2).
isOn(stone14,stone15).
isOn(stone15,hill3).
isOn(stone2,hill0).
isOn(stone2,stone1).
isOn(stone3,hill0).
isOn(stone3,stone2).
isOn(stone4,hill0).
isOn(stone4,stone3).
isOn(stone5,stone4).
isOn(stone7,stone5).
isOn(stone8,stone5).
isOn(stone9,stone5).
isOn(wood13,hill3).
isOn(wood13,ice12).
isOver(hill0,pig0).
isOver(ice12,pig0).
isOver(wood13,pig0).
isRight(ice12,stone11).
isRight(stone0,hill1).
isRight(stone1,hill1).
isRight(stone15,wood13).
isRight(stone2,hill1).
isRight(stone3,hill1).
isRight(stone4,hill1).
isRight(stone5,hill1).
isRight(stone7,stone0).
isRight(stone8,stone7).
isRight(stone9,stone8).
isTower(struct0).
isTower(struct1).
pig(pig0,504,342,10,10).
protects(struct3,pig0).
scene_scale(81,0.99524682).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 455.5,330.5,255,[5,51,1.57079633]).
shape(ice12,rect, 498,226.5,347,[7.05753347,49.25636317,1.75929189]).
shape(ice6,rect, 400,332,312,[6,52,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 392.5,262.5,175,[7,25,1.57079633]).
shape(stone1,rect, 402.5,304,150,[6,25,0]).
shape(stone11,rect, 473.5,248,150,[6,25,0]).
shape(stone14,rect, 541.5,192,150,[6,25,0]).
shape(stone15,rect, 542,198,144,[6,24,0]).
shape(stone2,rect, 402,297.5,168,[7,24,0]).
shape(stone3,rect, 402.5,291,150,[6,25,0]).
shape(stone4,rect, 402.5,284.5,174,[6.96893375,25.07726574,0.09424778]).
shape(stone5,rect, 403,278,144,[6,24,0]).
shape(stone7,rect, 401,262.5,150,[6,25,1.57079633]).
shape(stone8,rect, 407.5,262.5,175,[7,25,1.57079633]).
shape(stone9,rect, 414.5,262.5,175,[7,25,1.57079633]).
shape(wood13,rect, 514,198.5,153,[6.36490349,24.14101304,3.0787608]).
situation_name('situation1_0-4-9-10-10').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
