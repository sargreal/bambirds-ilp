belongsTo(ice11,struct1).
belongsTo(ice12,struct3).
belongsTo(ice13,struct3).
belongsTo(ice6,struct0).
belongsTo(pig0,struct2).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone10,struct0).
belongsTo(stone14,struct3).
belongsTo(stone16,struct3).
belongsTo(stone17,struct3).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(stone9,struct0).
belongsTo(wood15,struct3).
bird(redbird0).
bird(redbird1).
birdOrder(redbird0,0).
birdOrder(redbird1,1).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
ground_plane(384).
hasColor(redbird0,red).
hasColor(redbird1,red).
hasForm(ice11,bar).
hasForm(ice12,bar).
hasForm(ice13,bar).
hasForm(ice6,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone10,bar).
hasForm(stone14,bar).
hasForm(stone16,bar).
hasForm(stone17,bar).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone7,block).
hasForm(stone8,block).
hasForm(stone9,bar).
hasForm(wood15,bar).
hasMaterial(ice11,ice,453,305,5,51).
hasMaterial(ice12,ice,480,194,44,9).
hasMaterial(ice13,ice,487,202,9,49).
hasMaterial(ice6,ice,397,307,6,51).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,389,301,25,6).
hasMaterial(stone1,stone,389,293,25,9).
hasMaterial(stone10,stone,411,251,6,24).
hasMaterial(stone14,stone,492,165,6,24).
hasMaterial(stone16,stone,497,164,9,25).
hasMaterial(stone17,stone,507,163,5,25).
hasMaterial(stone2,stone,389,252,6,22).
hasMaterial(stone3,stone,390,288,25,6).
hasMaterial(stone4,stone,391,281,24,7).
hasMaterial(stone5,stone,391,275,25,6).
hasMaterial(stone7,stone,397,251,7,14).
hasMaterial(stone8,stone,398,265,5,10).
hasMaterial(stone9,stone,404,250,6,25).
hasMaterial(wood15,wood,493,189,25,6).
hasOrientation(ice11,vertical).
hasOrientation(ice12,horizontal).
hasOrientation(ice13,vertical).
hasOrientation(ice6,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone10,vertical).
hasOrientation(stone14,vertical).
hasOrientation(stone16,vertical).
hasOrientation(stone17,vertical).
hasOrientation(stone2,vertical).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(stone9,vertical).
hasOrientation(wood15,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice11,big).
hasSize(ice12,medium).
hasSize(ice13,big).
hasSize(ice6,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(redbird1,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone10,medium).
hasSize(stone14,medium).
hasSize(stone16,medium).
hasSize(stone17,medium).
hasSize(stone2,small).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(stone9,medium).
hasSize(wood15,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice11,struct1).
isAnchorPointFor(ice12,struct3).
isAnchorPointFor(pig0,struct2).
isAnchorPointFor(stone0,struct0).
isBelow(ice11,hill2).
isBelow(ice12,wood15).
isBelow(ice13,ice12).
isBelow(ice6,stone0).
isBelow(stone0,stone1).
isBelow(stone1,stone3).
isBelow(stone14,stone16).
isBelow(stone3,stone4).
isBelow(stone4,stone5).
isBelow(stone5,stone10).
isBelow(stone5,stone2).
isBelow(stone5,stone8).
isBelow(stone5,stone9).
isBelow(stone8,stone7).
isBelow(wood15,stone14).
isBelow(wood15,stone16).
isBelow(wood15,stone17).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isLeft(stone0,hill2).
isLeft(stone1,hill2).
isLeft(stone10,hill2).
isLeft(stone16,stone17).
isLeft(stone2,stone7).
isLeft(stone2,stone8).
isLeft(stone3,hill2).
isLeft(stone4,hill2).
isLeft(stone5,hill2).
isLeft(stone7,stone9).
isLeft(stone8,stone9).
isLeft(stone9,stone10).
isOn(ice11,hill0).
isOn(ice12,hill3).
isOn(ice12,ice13).
isOn(ice13,hill2).
isOn(ice6,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,ice6).
isOn(stone1,hill0).
isOn(stone1,stone0).
isOn(stone10,stone5).
isOn(stone14,wood15).
isOn(stone16,wood15).
isOn(stone17,wood15).
isOn(stone2,stone5).
isOn(stone3,hill0).
isOn(stone3,stone1).
isOn(stone4,hill0).
isOn(stone4,stone3).
isOn(stone5,stone4).
isOn(stone7,stone8).
isOn(stone8,stone5).
isOn(stone9,stone5).
isOn(wood15,ice12).
isOver(hill0,pig0).
isOver(ice12,pig0).
isOver(stone16,pig0).
isOver(stone17,pig0).
isOver(wood15,pig0).
isRight(stone0,hill1).
isRight(stone1,hill1).
isRight(stone10,stone9).
isRight(stone16,stone14).
isRight(stone17,stone16).
isRight(stone2,hill1).
isRight(stone3,hill1).
isRight(stone4,hill1).
isRight(stone5,hill1).
isRight(stone7,stone2).
isRight(stone8,stone2).
isRight(stone9,stone7).
isRight(stone9,stone8).
isTower(struct0).
isTower(struct1).
isTower(struct3).
pig(pig0,504,342,10,10).
protects(struct3,pig0).
scene_scale(81,0.99588921).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice11,rect, 455.5,330.5,255,[5,51,1.57079633]).
shape(ice12,rect, 502,198.5,303,[6.87654062,44.07252093,3.11017673]).
shape(ice13,rect, 491.5,226.5,331,[6.72178185,49.28005281,1.50796447]).
shape(ice6,rect, 400,332.5,306,[6,51,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(redbird1,ball, 129.5,375,103,[5.75]).
shape(stone0,rect, 401.5,304,150,[6,25,0]).
shape(stone1,rect, 401.5,297.5,174,[6.92339658,25.13903977,0.06283185]).
shape(stone10,rect, 414,263,144,[6,24,1.57079633]).
shape(stone14,rect, 495,177,144,[6,24,1.57079633]).
shape(stone16,rect, 501.5,176.5,168,[6.68660881,25.17137406,1.47654855]).
shape(stone17,rect, 509.5,175.5,125,[5,25,1.57079633]).
shape(stone2,poly, 393,263,132,[4,[389,257],[391,273],[394,274],[395,252]]).
shape(stone3,rect, 402.5,291,150,[6,25,0]).
shape(stone4,rect, 403,284.5,168,[7,24,0]).
shape(stone5,rect, 403.5,278,150,[6,25,0]).
shape(stone7,rect, 400.5,258,98,[7,14,1.57079633]).
shape(stone8,rect, 400.5,270,50,[5,10,1.57079633]).
shape(stone9,rect, 407,262.5,150,[6,25,1.57079633]).
shape(wood15,rect, 505.5,192,150,[6,25,0]).
situation_name('situation1_0-10-9-9').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
