belongsTo(ice10,struct2).
belongsTo(ice11,struct5).
belongsTo(ice12,struct3).
belongsTo(pig0,struct4).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone14,struct5).
belongsTo(stone15,struct5).
belongsTo(stone16,struct5).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(stone9,struct1).
belongsTo(wood13,struct5).
bird(redbird0).
bird(redbird1).
birdOrder(redbird0,0).
birdOrder(redbird1,1).
canCollapse(struct0,struct1).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct3,struct4).
canCollapse(struct4,struct3).
canCollapse(struct4,struct5).
canCollapse(struct5,struct4).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
collapsesInDirection(struct4,struct5,away).
collapsesInDirection(struct5,struct4,towards).
ground_plane(384).
hasColor(redbird0,red).
hasColor(redbird1,red).
hasForm(ice10,bar).
hasForm(ice11,bar).
hasForm(ice12,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone14,bar).
hasForm(stone15,bar).
hasForm(stone16,bar).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,cube).
hasForm(stone6,block).
hasForm(stone7,block).
hasForm(stone8,block).
hasForm(stone9,bar).
hasForm(wood13,bar).
hasMaterial(ice10,ice,454,306,9,50).
hasMaterial(ice11,ice,485,195,50,7).
hasMaterial(ice12,ice,491,242,4,9).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,379,331,24,6).
hasMaterial(stone1,stone,385,343,25,7).
hasMaterial(stone14,stone,498,164,8,24).
hasMaterial(stone15,stone,507,164,5,24).
hasMaterial(stone16,stone,515,163,5,25).
hasMaterial(stone2,stone,385,337,25,7).
hasMaterial(stone3,stone,388,350,25,6).
hasMaterial(stone4,stone,389,299,6,24).
hasMaterial(stone5,stone,396,322,10,10).
hasMaterial(stone6,stone,397,299,16,25).
hasMaterial(stone7,stone,403,325,17,11).
hasMaterial(stone8,stone,403,301,15,26).
hasMaterial(stone9,stone,424,347,25,6).
hasMaterial(wood13,wood,499,190,24,5).
hasOrientation(ice10,vertical).
hasOrientation(ice11,horizontal).
hasOrientation(ice12,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone14,vertical).
hasOrientation(stone15,vertical).
hasOrientation(stone16,vertical).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,vertical).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,vertical).
hasOrientation(stone7,horizontal).
hasOrientation(stone8,vertical).
hasOrientation(stone9,horizontal).
hasOrientation(wood13,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,big).
hasSize(ice11,big).
hasSize(ice12,small).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(redbird1,medium).
hasSize(stone0,big).
hasSize(stone1,big).
hasSize(stone14,big).
hasSize(stone15,big).
hasSize(stone16,big).
hasSize(stone2,big).
hasSize(stone3,big).
hasSize(stone4,big).
hasSize(stone5,small).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(stone9,big).
hasSize(wood13,big).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct2).
isAnchorPointFor(ice11,struct5).
isAnchorPointFor(ice12,struct3).
isAnchorPointFor(pig0,struct4).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone9,struct1).
isBelow(ice10,hill2).
isBelow(ice11,wood13).
isBelow(stone0,stone5).
isBelow(stone1,stone2).
isBelow(stone2,stone0).
isBelow(stone2,stone7).
isBelow(stone3,stone1).
isBelow(stone5,stone6).
isBelow(stone5,stone8).
isBelow(stone7,stone5).
isBelow(stone7,stone6).
isBelow(stone7,stone8).
isBelow(stone8,stone6).
isBelow(wood13,stone14).
isBelow(wood13,stone15).
isBelow(wood13,stone16).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isCollapsable(struct5).
isLeft(stone0,stone7).
isLeft(stone14,stone15).
isLeft(stone15,stone16).
isLeft(stone4,stone5).
isLeft(stone4,stone6).
isLeft(stone8,hill2).
isOn(ice10,hill0).
isOn(ice11,hill3).
isOn(ice12,hill2).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,stone2).
isOn(stone1,hill0).
isOn(stone1,stone3).
isOn(stone14,wood13).
isOn(stone15,wood13).
isOn(stone16,wood13).
isOn(stone2,hill0).
isOn(stone2,stone1).
isOn(stone3,hill0).
isOn(stone4,hill0).
isOn(stone5,hill0).
isOn(stone5,stone0).
isOn(stone5,stone7).
isOn(stone6,hill0).
isOn(stone6,stone5).
isOn(stone6,stone7).
isOn(stone6,stone8).
isOn(stone7,hill0).
isOn(stone7,stone2).
isOn(stone8,hill0).
isOn(stone8,stone5).
isOn(stone8,stone7).
isOn(stone9,hill0).
isOn(wood13,ice11).
isOver(hill0,pig0).
isOver(ice11,pig0).
isOver(stone14,pig0).
isOver(stone15,pig0).
isOver(stone16,pig0).
isOver(wood13,pig0).
isRight(stone15,stone14).
isRight(stone16,stone15).
isRight(stone4,hill1).
isRight(stone5,stone4).
isRight(stone6,stone4).
isRight(stone7,stone0).
isTower(struct0).
isTower(struct2).
isTower(struct3).
pig(pig0,504,342,10,10).
protects(struct5,pig0).
scene_scale(81,0.99634451).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 458.5,331,327,[6.53102227,50.0695603,1.5393804]).
shape(ice11,rect, 510,198.5,350,[7,50,0]).
shape(ice12,poly, 496,233,36,[3,[491,242],[491,249],[495,251]]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(redbird1,ball, 129.5,375.5,95,[5.5]).
shape(stone0,rect, 391,334,144,[6,24,0]).
shape(stone1,rect, 397.5,346.5,175,[7,25,0]).
shape(stone14,rect, 502,176,136,[5.67420777,24.14101304,1.50796447]).
shape(stone15,rect, 509.5,176,120,[5,24,1.57079633]).
shape(stone16,rect, 517.5,175.5,125,[5,25,1.57079633]).
shape(stone2,rect, 397.5,340.5,175,[7,25,0]).
shape(stone3,rect, 400.5,353,150,[6,25,0]).
shape(stone4,rect, 392,311,144,[6,24,1.57079633]).
shape(stone5,rect, 400.5,326.5,49,[6.36396103,7.77817459,0.78539816]).
shape(stone6,rect, 405,311.5,163,[6.5084354,25.0661047,1.94778745]).
shape(stone7,rect, 411.5,330.5,90,[5.72061761,15.80690114,0.34557519]).
shape(stone8,rect, 410.5,314,159,[6.31490802,25.25963208,1.94778745]).
shape(stone9,rect, 436.5,350,150,[6,25,0]).
shape(wood13,poly, 512,192,60,[4,[499,190],[501,195],[522,194],[523,190]]).
situation_name('situation1_0-7-4-10').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
structure(struct5).
