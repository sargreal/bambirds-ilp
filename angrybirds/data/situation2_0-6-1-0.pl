belongsTo(ice11,struct2).
belongsTo(ice14,struct4).
belongsTo(ice15,struct4).
belongsTo(ice16,struct4).
belongsTo(pig0,struct3).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone10,struct1).
belongsTo(stone13,struct4).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(stone9,struct1).
belongsTo(wood12,struct4).
bird(redbird0).
bird(redbird1).
birdOrder(redbird0,0).
birdOrder(redbird1,1).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct4).
canCollapse(struct4,struct3).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasColor(redbird1,red).
hasForm(ice11,bar).
hasForm(ice14,bar).
hasForm(ice15,block).
hasForm(ice16,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone10,bar).
hasForm(stone13,block).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(stone9,bar).
hasForm(wood12,block).
hasMaterial(ice11,ice,455,305,6,51).
hasMaterial(ice14,ice,516,239,5,32).
hasMaterial(ice15,ice,516,234,8,12).
hasMaterial(ice16,ice,517,220,5,14).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,388,343,25,7).
hasMaterial(stone1,stone,389,350,24,6).
hasMaterial(stone10,stone,450,238,25,6).
hasMaterial(stone13,stone,492,233,24,18).
hasMaterial(stone2,stone,389,337,24,6).
hasMaterial(stone3,stone,389,299,6,25).
hasMaterial(stone4,stone,390,330,24,7).
hasMaterial(stone5,stone,392,324,24,6).
hasMaterial(stone6,stone,398,299,6,25).
hasMaterial(stone7,stone,404,300,7,24).
hasMaterial(stone8,stone,411,300,6,24).
hasMaterial(stone9,stone,442,245,24,6).
hasMaterial(wood12,wood,485,230,21,18).
hasOrientation(ice11,vertical).
hasOrientation(ice14,vertical).
hasOrientation(ice15,vertical).
hasOrientation(ice16,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone10,horizontal).
hasOrientation(stone13,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,vertical).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,vertical).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(stone9,horizontal).
hasOrientation(wood12,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice11,big).
hasSize(ice14,big).
hasSize(ice15,medium).
hasSize(ice16,small).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(redbird1,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone10,medium).
hasSize(stone13,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(stone9,medium).
hasSize(wood12,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice11,struct2).
isAnchorPointFor(pig0,struct3).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone9,struct1).
isAnchorPointFor(wood12,struct4).
isBelow(ice11,hill2).
isBelow(ice14,ice15).
isBelow(ice15,ice16).
isBelow(stone0,stone2).
isBelow(stone1,stone0).
isBelow(stone13,wood12).
isBelow(stone2,stone4).
isBelow(stone4,stone5).
isBelow(stone5,stone3).
isBelow(stone5,stone6).
isBelow(stone5,stone7).
isBelow(stone5,stone8).
isBelow(stone9,stone10).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(ice14,hill3).
isLeft(ice16,hill3).
isLeft(stone13,ice14).
isLeft(stone13,ice15).
isLeft(stone13,ice16).
isLeft(stone3,stone6).
isLeft(stone6,stone7).
isLeft(stone7,stone8).
isLeft(stone8,hill2).
isOn(ice11,hill0).
isOn(ice14,ground).
isOn(ice15,hill3).
isOn(ice15,ice14).
isOn(ice16,ice15).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,stone1).
isOn(stone1,hill0).
isOn(stone10,stone9).
isOn(stone13,hill2).
isOn(stone2,hill0).
isOn(stone2,stone0).
isOn(stone3,hill0).
isOn(stone3,stone5).
isOn(stone4,hill0).
isOn(stone4,stone2).
isOn(stone5,hill0).
isOn(stone5,stone4).
isOn(stone6,hill0).
isOn(stone6,stone5).
isOn(stone7,hill0).
isOn(stone7,stone5).
isOn(stone8,hill0).
isOn(stone8,stone5).
isOn(stone9,hill2).
isOn(wood12,hill2).
isOn(wood12,stone13).
isOver(hill0,pig0).
isOver(ice14,pig0).
isOver(ice15,pig0).
isOver(ice16,pig0).
isOver(stone13,pig0).
isOver(wood12,pig0).
isRight(ice14,stone13).
isRight(ice15,stone13).
isRight(ice16,stone13).
isRight(stone3,hill1).
isRight(stone6,stone3).
isRight(stone7,stone6).
isRight(stone8,stone7).
isTower(struct0).
isTower(struct2).
isTower(struct4).
pig(pig0,504,342,10,10).
protects(struct4,pig0).
scene_scale(81,0.99363239).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice11,rect, 458,330.5,306,[6,51,1.57079633]).
shape(ice14,poly, 519,255,160,[3,[516,239],[517,270],[521,271]]).
shape(ice15,rect, 520,240,50,[4.95713917,10.16091907,1.28805299]).
shape(ice16,rect, 519.5,227,70,[5,14,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(redbird1,ball, 129.5,375,103,[5.75]).
shape(stone0,rect, 400.5,346.5,175,[7,25,0]).
shape(stone1,rect, 401,353,144,[6,24,0]).
shape(stone10,rect, 462.5,241,150,[6,25,0]).
shape(stone13,rect, 504,242,143,[5.75747503,24.94077317,2.54469005]).
shape(stone2,rect, 401,340,144,[6,24,0]).
shape(stone3,rect, 392,311.5,150,[6,25,1.57079633]).
shape(stone4,rect, 402,333.5,168,[7,24,0]).
shape(stone5,rect, 404,327,144,[6,24,0]).
shape(stone6,rect, 401,311.5,150,[6,25,1.57079633]).
shape(stone7,rect, 407.5,312,168,[7,24,1.57079633]).
shape(stone8,rect, 414,312,144,[6,24,1.57079633]).
shape(stone9,rect, 454,248,144,[6,24,0]).
shape(wood12,rect, 495.5,239,105,[4.41163848,23.82154817,2.51327412]).
situation_name('situation1_0-6-1-0').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
