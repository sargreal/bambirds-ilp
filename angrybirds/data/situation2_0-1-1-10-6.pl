belongsTo(ice11,struct4).
belongsTo(ice9,struct1).
belongsTo(pig0,struct3).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone10,struct2).
belongsTo(stone13,struct4).
belongsTo(stone14,struct4).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(wood12,struct4).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct4).
canCollapse(struct4,struct3).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice11,bar).
hasForm(ice9,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone10,bar).
hasForm(stone13,bar).
hasForm(stone14,bar).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(wood12,bar).
hasMaterial(ice11,ice,490,201,14,50).
hasMaterial(ice9,ice,454,306,7,44).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,389,337,25,7).
hasMaterial(stone1,stone,389,331,25,7).
hasMaterial(stone10,stone,460,245,25,6).
hasMaterial(stone13,stone,509,171,6,24).
hasMaterial(stone14,stone,516,170,6,25).
hasMaterial(stone2,stone,389,325,25,6).
hasMaterial(stone3,stone,389,300,6,25).
hasMaterial(stone4,stone,390,350,25,6).
hasMaterial(stone5,stone,390,344,25,6).
hasMaterial(stone6,stone,395,300,6,25).
hasMaterial(stone7,stone,403,300,6,25).
hasMaterial(stone8,stone,411,301,6,24).
hasMaterial(wood12,wood,501,196,25,6).
hasOrientation(ice11,vertical).
hasOrientation(ice9,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone10,horizontal).
hasOrientation(stone13,vertical).
hasOrientation(stone14,vertical).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,vertical).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,vertical).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(wood12,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice11,big).
hasSize(ice9,medium).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone10,medium).
hasSize(stone13,small).
hasSize(stone14,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,small).
hasSize(wood12,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice11,struct4).
isAnchorPointFor(ice9,struct1).
isAnchorPointFor(pig0,struct3).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone10,struct2).
isBelow(ice11,wood12).
isBelow(ice9,hill2).
isBelow(stone0,stone1).
isBelow(stone1,stone2).
isBelow(stone2,stone3).
isBelow(stone2,stone6).
isBelow(stone2,stone7).
isBelow(stone2,stone8).
isBelow(stone4,stone5).
isBelow(stone5,stone0).
isBelow(wood12,stone13).
isBelow(wood12,stone14).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(stone13,stone14).
isLeft(stone3,stone6).
isLeft(stone6,stone7).
isLeft(stone7,stone8).
isLeft(stone8,hill2).
isOn(ice11,hill2).
isOn(ice9,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,stone5).
isOn(stone1,hill0).
isOn(stone1,stone0).
isOn(stone10,hill2).
isOn(stone13,wood12).
isOn(stone14,wood12).
isOn(stone2,hill0).
isOn(stone2,stone1).
isOn(stone3,hill0).
isOn(stone3,stone2).
isOn(stone4,hill0).
isOn(stone5,hill0).
isOn(stone5,stone4).
isOn(stone6,hill0).
isOn(stone6,stone2).
isOn(stone7,hill0).
isOn(stone7,stone2).
isOn(stone8,hill0).
isOn(stone8,stone2).
isOn(wood12,hill3).
isOn(wood12,ice11).
isOver(hill0,pig0).
isOver(ice11,pig0).
isOver(stone13,pig0).
isOver(stone14,pig0).
isOver(wood12,pig0).
isRight(stone14,stone13).
isRight(stone3,hill1).
isRight(stone6,stone3).
isRight(stone7,stone6).
isRight(stone8,stone7).
isTower(struct0).
isTower(struct1).
isTower(struct4).
pig(pig0,504,342,10,10).
protects(struct4,pig0).
scene_scale(81,1.0006715).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice11,rect, 497,226,361,[7.20602054,50.23240123,1.69646003]).
shape(ice9,poly, 458,331,308,[3,[454,306],[458,350],[461,346]]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 401.5,340.5,175,[7,25,0]).
shape(stone1,rect, 401.5,334.5,172,[6.87090289,25.08189629,0.03141593]).
shape(stone10,rect, 472.5,248,150,[6,25,0]).
shape(stone13,rect, 512,183,144,[6,24,1.57079633]).
shape(stone14,rect, 519,182.5,149,[5.9656286,25.01907477,1.5393804]).
shape(stone2,rect, 401.5,328,150,[6,25,0]).
shape(stone3,rect, 392,312.5,150,[6,25,1.57079633]).
shape(stone4,rect, 402.5,353,150,[6,25,0]).
shape(stone5,rect, 402.5,347,150,[6,25,0]).
shape(stone6,rect, 398,312.5,150,[6,25,1.57079633]).
shape(stone7,rect, 406,312.5,150,[6,25,1.57079633]).
shape(stone8,rect, 414,313,144,[6,24,1.57079633]).
shape(wood12,rect, 513.5,199,150,[6,25,0]).
situation_name('situation1_0-1-1-10-6').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
