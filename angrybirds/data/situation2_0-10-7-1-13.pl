belongsTo(ice10,struct2).
belongsTo(ice11,struct4).
belongsTo(ice9,struct1).
belongsTo(pig0,struct3).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone13,struct5).
belongsTo(stone14,struct5).
belongsTo(stone15,struct5).
belongsTo(stone16,struct5).
belongsTo(stone17,struct5).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(wood12,struct4).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct3,struct4).
canCollapse(struct4,struct3).
canCollapse(struct4,struct5).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
collapsesInDirection(struct4,struct5,away).
collapsesInDirection(struct5,struct4,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,block).
hasForm(ice11,bar).
hasForm(ice9,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone13,block).
hasForm(stone14,ball).
hasForm(stone15,bar).
hasForm(stone16,bar).
hasForm(stone17,block).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(wood12,bar).
hasMaterial(ice10,ice,492,210,29,38).
hasMaterial(ice11,ice,512,196,46,6).
hasMaterial(ice9,ice,455,305,6,37).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,388,350,25,6).
hasMaterial(stone1,stone,388,343,24,7).
hasMaterial(stone13,stone,568,369,26,14).
hasMaterial(stone14,stone,593,377,4,4).
hasMaterial(stone15,stone,598,377,16,6).
hasMaterial(stone16,stone,615,373,14,6).
hasMaterial(stone17,stone,627,376,7,5).
hasMaterial(stone2,stone,389,336,24,7).
hasMaterial(stone3,stone,390,330,24,7).
hasMaterial(stone4,stone,390,299,6,25).
hasMaterial(stone5,stone,391,324,25,6).
hasMaterial(stone6,stone,397,299,7,25).
hasMaterial(stone7,stone,404,299,7,25).
hasMaterial(stone8,stone,411,299,6,25).
hasMaterial(wood12,wood,530,189,23,5).
hasOrientation(ice10,vertical).
hasOrientation(ice11,horizontal).
hasOrientation(ice9,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone13,horizontal).
hasOrientation(stone14,horizontal).
hasOrientation(stone15,horizontal).
hasOrientation(stone16,horizontal).
hasOrientation(stone17,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,vertical).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,vertical).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(wood12,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,medium).
hasSize(ice11,big).
hasSize(ice9,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone13,medium).
hasSize(stone14,medium).
hasSize(stone15,medium).
hasSize(stone16,small).
hasSize(stone17,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(wood12,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct2).
isAnchorPointFor(ice11,struct4).
isAnchorPointFor(ice9,struct1).
isAnchorPointFor(pig0,struct3).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone13,struct5).
isBelow(ice11,wood12).
isBelow(ice9,hill2).
isBelow(stone0,stone1).
isBelow(stone1,stone2).
isBelow(stone17,stone16).
isBelow(stone2,stone3).
isBelow(stone3,stone5).
isBelow(stone5,stone4).
isBelow(stone5,stone6).
isBelow(stone5,stone7).
isBelow(stone5,stone8).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isCollapsable(struct5).
isLeft(ice10,hill3).
isLeft(stone13,stone14).
isLeft(stone13,stone15).
isLeft(stone14,stone15).
isLeft(stone15,stone16).
isLeft(stone4,stone6).
isLeft(stone6,stone7).
isLeft(stone7,stone8).
isLeft(stone8,hill2).
isOn(ice10,hill2).
isOn(ice11,hill3).
isOn(ice9,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone1,hill0).
isOn(stone1,stone0).
isOn(stone13,ground).
isOn(stone14,stone13).
isOn(stone15,ground).
isOn(stone16,stone17).
isOn(stone17,ground).
isOn(stone2,hill0).
isOn(stone2,stone1).
isOn(stone3,hill0).
isOn(stone3,stone2).
isOn(stone4,hill0).
isOn(stone4,stone5).
isOn(stone5,hill0).
isOn(stone5,stone3).
isOn(stone6,hill0).
isOn(stone6,stone5).
isOn(stone7,hill0).
isOn(stone7,stone5).
isOn(stone8,hill0).
isOn(stone8,stone5).
isOn(wood12,ice11).
isOver(hill0,pig0).
isOver(ice10,pig0).
isOver(ice11,pig0).
isOver(stone13,stone14).
isRight(stone15,stone13).
isRight(stone15,stone14).
isRight(stone16,stone15).
isRight(stone4,hill1).
isRight(stone6,stone4).
isRight(stone7,stone6).
isRight(stone8,stone7).
isTower(struct0).
isTower(struct1).
isTower(struct2).
pig(pig0,504,342,10,10).
protects(struct2,pig0).
protects(struct4,pig0).
scene_scale(81,0.99543454).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 506.5,229,234,[5.44188722,43.01022611,2.19911486]).
shape(ice11,rect, 535,199,276,[6,46,0]).
shape(ice9,rect, 458,323.5,222,[6,37,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 400.5,353,150,[6,25,0]).
shape(stone1,rect, 400,346.5,168,[7,24,0]).
shape(stone13,rect, 581,376,155,[6.08737269,25.60650934,2.82743339]).
shape(stone14,ball, 595.5,380,28,[3]).
shape(stone15,rect, 606,380,96,[6,16,0]).
shape(stone16,poly, 622,376,84,[4,[615,373],[617,377],[626,379],[629,376]]).
shape(stone17,rect, 630.5,378.5,35,[5,7,0]).
shape(stone2,rect, 401,339.5,168,[7,24,0]).
shape(stone3,rect, 402,333.5,168,[7,24,0]).
shape(stone4,rect, 393,311.5,150,[6,25,1.57079633]).
shape(stone5,rect, 403.5,327,150,[6,25,0]).
shape(stone6,rect, 400.5,311.5,175,[7,25,1.57079633]).
shape(stone7,rect, 407.5,311.5,175,[7,25,1.57079633]).
shape(stone8,rect, 414,311.5,150,[6,25,1.57079633]).
shape(wood12,rect, 541.5,191.5,115,[5,23,0]).
situation_name('situation1_0-10-7-1-13').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
structure(struct5).
