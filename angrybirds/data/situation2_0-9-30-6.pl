belongsTo(ice10,struct1).
belongsTo(ice11,struct3).
belongsTo(ice12,struct3).
belongsTo(pig0,struct2).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone13,struct3).
belongsTo(stone15,struct3).
belongsTo(stone16,struct3).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(stone9,struct0).
belongsTo(wood14,struct3).
bird(redbird0).
bird(redbird1).
birdOrder(redbird0,0).
birdOrder(redbird1,1).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
ground_plane(384).
hasColor(redbird0,red).
hasColor(redbird1,red).
hasForm(ice10,bar).
hasForm(ice11,bar).
hasForm(ice12,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone13,bar).
hasForm(stone15,bar).
hasForm(stone16,bar).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,block).
hasForm(stone8,bar).
hasForm(stone9,bar).
hasForm(wood14,bar).
hasMaterial(ice10,ice,455,306,6,50).
hasMaterial(ice11,ice,480,194,44,9).
hasMaterial(ice12,ice,487,202,9,49).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,384,330,25,7).
hasMaterial(stone1,stone,387,343,24,7).
hasMaterial(stone13,stone,492,165,6,24).
hasMaterial(stone15,stone,497,164,9,25).
hasMaterial(stone16,stone,507,163,5,25).
hasMaterial(stone2,stone,387,337,25,7).
hasMaterial(stone3,stone,389,350,15,6).
hasMaterial(stone4,stone,389,299,6,25).
hasMaterial(stone5,stone,393,324,25,7).
hasMaterial(stone6,stone,397,299,7,25).
hasMaterial(stone7,stone,404,350,10,6).
hasMaterial(stone8,stone,405,300,8,25).
hasMaterial(stone9,stone,419,328,6,25).
hasMaterial(wood14,wood,493,189,25,6).
hasOrientation(ice10,vertical).
hasOrientation(ice11,horizontal).
hasOrientation(ice12,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone13,vertical).
hasOrientation(stone15,vertical).
hasOrientation(stone16,vertical).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,vertical).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,vertical).
hasOrientation(stone7,horizontal).
hasOrientation(stone8,vertical).
hasOrientation(stone9,vertical).
hasOrientation(wood14,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,big).
hasSize(ice11,big).
hasSize(ice12,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(redbird1,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone13,medium).
hasSize(stone15,medium).
hasSize(stone16,medium).
hasSize(stone2,medium).
hasSize(stone3,small).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(stone9,medium).
hasSize(wood14,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct1).
isAnchorPointFor(ice11,struct3).
isAnchorPointFor(pig0,struct2).
isAnchorPointFor(stone0,struct0).
isBelow(ice10,hill2).
isBelow(ice11,wood14).
isBelow(ice12,ice11).
isBelow(stone0,stone5).
isBelow(stone1,stone2).
isBelow(stone13,stone15).
isBelow(stone2,stone0).
isBelow(stone3,stone1).
isBelow(stone5,stone4).
isBelow(stone5,stone6).
isBelow(stone5,stone8).
isBelow(stone7,stone1).
isBelow(wood14,stone13).
isBelow(wood14,stone15).
isBelow(wood14,stone16).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isLeft(stone15,stone16).
isLeft(stone3,stone7).
isLeft(stone4,stone6).
isLeft(stone5,stone9).
isLeft(stone6,stone8).
isOn(ice10,hill0).
isOn(ice11,hill3).
isOn(ice11,ice12).
isOn(ice12,hill2).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,stone2).
isOn(stone1,hill0).
isOn(stone1,stone3).
isOn(stone1,stone7).
isOn(stone13,wood14).
isOn(stone15,wood14).
isOn(stone16,wood14).
isOn(stone2,hill0).
isOn(stone2,stone1).
isOn(stone3,hill0).
isOn(stone4,hill0).
isOn(stone4,stone5).
isOn(stone5,hill0).
isOn(stone5,stone0).
isOn(stone6,hill0).
isOn(stone6,stone5).
isOn(stone7,hill0).
isOn(stone8,hill0).
isOn(stone8,stone5).
isOn(stone9,hill0).
isOn(wood14,ice11).
isOver(hill0,pig0).
isOver(ice11,pig0).
isOver(stone15,pig0).
isOver(stone16,pig0).
isOver(wood14,pig0).
isRight(stone15,stone13).
isRight(stone16,stone15).
isRight(stone4,hill1).
isRight(stone6,stone4).
isRight(stone7,stone3).
isRight(stone8,stone6).
isRight(stone9,stone5).
isTower(struct0).
isTower(struct1).
isTower(struct3).
pig(pig0,504,342,10,10).
protects(struct3,pig0).
scene_scale(81,0.99660898).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 458,331,300,[6,50,1.57079633]).
shape(ice11,rect, 502,198.5,303,[6.87654062,44.07252093,3.11017673]).
shape(ice12,rect, 491.5,226.5,331,[6.72178185,49.28005281,1.50796447]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(redbird1,ball, 129.5,375,103,[5.75]).
shape(stone0,rect, 396.5,333.5,175,[7,25,0]).
shape(stone1,rect, 399,346.5,168,[7,24,0]).
shape(stone13,rect, 495,177,144,[6,24,1.57079633]).
shape(stone15,rect, 501.5,176.5,168,[6.68660881,25.17137406,1.47654855]).
shape(stone16,rect, 509.5,175.5,125,[5,25,1.57079633]).
shape(stone2,rect, 399.5,340.5,175,[7,25,0]).
shape(stone3,rect, 396.5,353,90,[6,15,0]).
shape(stone4,rect, 392,311.5,150,[6,25,1.57079633]).
shape(stone5,rect, 405.5,327.5,175,[7,25,0]).
shape(stone6,rect, 400.5,311.5,155,[6.16991035,25.20183029,1.63362818]).
shape(stone7,rect, 409,353,60,[6,10,0]).
shape(stone8,rect, 409,312.5,148,[5.92536985,25.13903977,1.63362818]).
shape(stone9,rect, 422,340.5,150,[6,25,1.57079633]).
shape(wood14,rect, 505.5,192,150,[6,25,0]).
situation_name('situation1_0-9-30-6').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
