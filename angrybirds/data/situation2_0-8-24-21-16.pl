belongsTo(ice10,struct3).
belongsTo(ice11,struct3).
belongsTo(ice14,struct3).
belongsTo(ice9,struct1).
belongsTo(pig0,struct2).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone13,struct3).
belongsTo(stone15,struct4).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(wood12,struct3).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct3,struct4).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,bar).
hasForm(ice11,bar).
hasForm(ice14,bar).
hasForm(ice9,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone13,block).
hasForm(stone15,bar).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(wood12,bar).
hasMaterial(ice10,ice,488,195,21,6).
hasMaterial(ice11,ice,491,202,7,49).
hasMaterial(ice14,ice,523,196,13,6).
hasMaterial(ice9,ice,455,306,6,50).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,388,300,9,26).
hasMaterial(stone1,stone,390,325,25,7).
hasMaterial(stone13,stone,517,178,22,18).
hasMaterial(stone15,stone,561,377,25,6).
hasMaterial(stone2,stone,391,344,25,6).
hasMaterial(stone3,stone,391,331,25,7).
hasMaterial(stone4,stone,392,350,24,6).
hasMaterial(stone5,stone,392,337,25,7).
hasMaterial(stone6,stone,396,300,6,25).
hasMaterial(stone7,stone,403,300,6,25).
hasMaterial(stone8,stone,410,300,6,25).
hasMaterial(wood12,wood,502,189,21,6).
hasOrientation(ice10,horizontal).
hasOrientation(ice11,vertical).
hasOrientation(ice14,horizontal).
hasOrientation(ice9,vertical).
hasOrientation(stone0,vertical).
hasOrientation(stone1,horizontal).
hasOrientation(stone13,horizontal).
hasOrientation(stone15,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone6,vertical).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(wood12,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,medium).
hasSize(ice11,big).
hasSize(ice14,small).
hasSize(ice9,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone13,medium).
hasSize(stone15,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,medium).
hasSize(wood12,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct3).
isAnchorPointFor(ice9,struct1).
isAnchorPointFor(pig0,struct2).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone15,struct4).
isBelow(ice10,wood12).
isBelow(ice11,ice10).
isBelow(ice14,stone13).
isBelow(ice9,hill2).
isBelow(stone1,stone0).
isBelow(stone1,stone6).
isBelow(stone1,stone7).
isBelow(stone1,stone8).
isBelow(stone2,stone5).
isBelow(stone3,stone1).
isBelow(stone4,stone2).
isBelow(stone5,stone3).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(stone0,stone6).
isLeft(stone6,stone7).
isLeft(stone7,stone8).
isLeft(stone8,hill2).
isOn(ice10,ice11).
isOn(ice11,hill2).
isOn(ice14,hill3).
isOn(ice9,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,stone1).
isOn(stone1,hill0).
isOn(stone1,stone3).
isOn(stone13,ice14).
isOn(stone15,ground).
isOn(stone2,hill0).
isOn(stone2,stone4).
isOn(stone3,hill0).
isOn(stone3,stone5).
isOn(stone4,hill0).
isOn(stone5,hill0).
isOn(stone5,stone2).
isOn(stone6,hill0).
isOn(stone6,stone0).
isOn(stone6,stone1).
isOn(stone7,hill0).
isOn(stone7,stone1).
isOn(stone8,hill0).
isOn(stone8,stone1).
isOn(wood12,ice10).
isOn(wood12,stone13).
isOver(hill0,pig0).
isOver(ice10,pig0).
isOver(stone13,pig0).
isOver(wood12,pig0).
isRight(stone0,hill1).
isRight(stone13,wood12).
isRight(stone15,hill0).
isRight(stone7,stone6).
isRight(stone8,stone7).
isTower(struct0).
isTower(struct1).
isTower(struct3).
pig(pig0,504,342,10,10).
protects(struct3,pig0).
scene_scale(81,0.99585224).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,poly, 499,198,126,[6,[488,200],[489,201],[509,198],[508,195],[499,196],[495,199]]).
shape(ice11,rect, 494.5,226.5,308,[6.27973619,49.10146449,1.60221225]).
shape(ice14,rect, 529.5,199,50,[3.90366705,12.90506115,0.21991149]).
shape(ice9,rect, 458,331,300,[6,50,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 392.5,313,166,[6.43431559,25.94869494,1.50796447]).
shape(stone1,rect, 402.5,328.5,172,[6.90231365,24.98766401,3.11017673]).
shape(stone13,rect, 528,187,112,[4.62502296,24.41077878,0.5969026]).
shape(stone15,rect, 573.5,380,150,[6,25,0]).
shape(stone2,rect, 403.5,347,150,[6,25,0]).
shape(stone3,rect, 403.5,334.5,175,[7,25,0]).
shape(stone4,rect, 404,353,144,[6,24,0]).
shape(stone5,rect, 404.5,340.5,174,[6.96513516,25.05048553,3.11017673]).
shape(stone6,rect, 399,312.5,150,[6,25,1.57079633]).
shape(stone7,rect, 406,312.5,150,[6,25,1.57079633]).
shape(stone8,rect, 413,312.5,150,[6,25,1.57079633]).
shape(wood12,rect, 512.5,192,126,[6,21,0]).
situation_name('situation1_0-8-24-21-16').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
