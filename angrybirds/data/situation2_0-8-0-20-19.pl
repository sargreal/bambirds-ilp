belongsTo(ice10,struct3).
belongsTo(ice8,struct1).
belongsTo(ice9,struct3).
belongsTo(pig0,struct2).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone12,struct3).
belongsTo(stone13,struct4).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct1).
belongsTo(wood11,struct3).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct3,struct4).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,bar).
hasForm(ice8,bar).
hasForm(ice9,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone12,block).
hasForm(stone13,bar).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone6,bar).
hasForm(stone7,bar).
hasForm(wood11,bar).
hasMaterial(ice10,ice,491,202,7,49).
hasMaterial(ice8,ice,455,306,6,50).
hasMaterial(ice9,ice,488,196,21,5).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,389,350,24,6).
hasMaterial(stone1,stone,389,343,24,6).
hasMaterial(stone12,stone,526,183,22,19).
hasMaterial(stone13,stone,561,377,25,6).
hasMaterial(stone2,stone,390,336,25,7).
hasMaterial(stone3,stone,391,330,24,7).
hasMaterial(stone4,stone,394,305,9,25).
hasMaterial(stone5,stone,400,305,11,25).
hasMaterial(stone6,stone,408,305,9,25).
hasMaterial(stone7,stone,427,347,25,6).
hasMaterial(wood11,wood,502,189,22,6).
hasOrientation(ice10,vertical).
hasOrientation(ice8,vertical).
hasOrientation(ice9,horizontal).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone12,horizontal).
hasOrientation(stone13,horizontal).
hasOrientation(stone2,horizontal).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,vertical).
hasOrientation(stone5,vertical).
hasOrientation(stone6,vertical).
hasOrientation(stone7,horizontal).
hasOrientation(wood11,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,big).
hasSize(ice8,big).
hasSize(ice9,small).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone12,medium).
hasSize(stone13,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(wood11,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice9,struct3).
isAnchorPointFor(pig0,struct2).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone13,struct4).
isAnchorPointFor(stone7,struct1).
isBelow(ice10,ice9).
isBelow(ice8,hill2).
isBelow(ice9,wood11).
isBelow(stone0,stone1).
isBelow(stone1,stone2).
isBelow(stone2,stone3).
isBelow(stone3,stone4).
isBelow(stone3,stone5).
isBelow(stone3,stone6).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(stone4,stone5).
isLeft(stone5,stone6).
isLeft(stone6,hill2).
isLeft(stone7,ice8).
isLeft(wood11,stone12).
isOn(ice10,hill2).
isOn(ice8,hill0).
isOn(ice9,ice10).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone1,hill0).
isOn(stone1,stone0).
isOn(stone12,hill3).
isOn(stone13,ground).
isOn(stone2,hill0).
isOn(stone2,stone1).
isOn(stone3,hill0).
isOn(stone3,stone2).
isOn(stone4,hill0).
isOn(stone4,stone3).
isOn(stone5,hill0).
isOn(stone5,stone3).
isOn(stone6,hill0).
isOn(stone6,stone3).
isOn(stone7,hill0).
isOn(wood11,ice9).
isOver(hill0,pig0).
isOver(ice9,pig0).
isOver(wood11,pig0).
isRight(ice8,stone7).
isRight(stone12,wood11).
isRight(stone13,hill0).
isRight(stone5,stone4).
isRight(stone6,stone5).
isTower(struct0).
isTower(struct1).
isTower(struct3).
pig(pig0,504,342,10,10).
protects(struct3,pig0).
scene_scale(81,0.98331735).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 494.5,226.5,314,[6.3997415,49.13287525,1.60221225]).
shape(ice8,rect, 458,331,300,[6,50,1.57079633]).
shape(ice9,poly, 500,198,52.5,[6,[488,200],[489,201],[509,199],[508,196],[499,196],[495,199]]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 401,353,144,[6,24,0]).
shape(stone1,rect, 401,346,144,[6,24,0]).
shape(stone12,rect, 537,192.5,111,[4.49404664,24.77588693,0.62831853]).
shape(stone13,rect, 573.5,380,150,[6,25,0]).
shape(stone2,rect, 402.5,339.5,175,[7,25,0]).
shape(stone3,rect, 403,333.5,168,[7,24,0]).
shape(stone4,rect, 398.5,317.5,168,[6.70468761,25.17886723,1.69646003]).
shape(stone5,rect, 405.5,317.5,173,[6.91381838,25.16151191,1.72787596]).
shape(stone6,rect, 412.5,317.5,165,[6.5925005,25.17137406,1.66504411]).
shape(stone7,rect, 439.5,350,150,[6,25,0]).
shape(wood11,rect, 513,192,132,[6,22,0]).
situation_name('situation1_0-8-0-20-19').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
