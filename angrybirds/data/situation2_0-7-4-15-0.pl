belongsTo(ice10,struct1).
belongsTo(ice15,struct2).
belongsTo(ice16,struct4).
belongsTo(ice6,struct0).
belongsTo(pig0,struct3).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone11,struct2).
belongsTo(stone12,struct2).
belongsTo(stone14,struct2).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct0).
belongsTo(stone9,struct0).
belongsTo(wood13,struct2).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct1,struct2).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct3,struct4).
canCollapse(struct4,struct3).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,bar).
hasForm(ice15,block).
hasForm(ice16,cube).
hasForm(ice6,bar).
hasForm(stone0,bar).
hasForm(stone1,bar).
hasForm(stone11,block).
hasForm(stone12,block).
hasForm(stone14,block).
hasForm(stone2,bar).
hasForm(stone3,bar).
hasForm(stone4,bar).
hasForm(stone5,bar).
hasForm(stone7,bar).
hasForm(stone8,bar).
hasForm(stone9,bar).
hasForm(wood13,block).
hasMaterial(ice10,ice,453,305,5,51).
hasMaterial(ice15,ice,484,208,33,44).
hasMaterial(ice16,ice,515,211,0,0).
hasMaterial(ice6,ice,394,307,7,51).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,389,300,25,8).
hasMaterial(stone1,stone,389,293,25,9).
hasMaterial(stone11,stone,458,233,24,17).
hasMaterial(stone12,stone,461,220,23,21).
hasMaterial(stone14,stone,483,203,18,23).
hasMaterial(stone2,stone,388,249,9,25).
hasMaterial(stone3,stone,390,286,25,10).
hasMaterial(stone4,stone,391,281,24,7).
hasMaterial(stone5,stone,392,274,24,9).
hasMaterial(stone7,stone,397,250,7,25).
hasMaterial(stone8,stone,402,251,10,24).
hasMaterial(stone9,stone,409,251,9,25).
hasMaterial(wood13,wood,482,217,18,24).
hasOrientation(ice10,vertical).
hasOrientation(ice15,vertical).
hasOrientation(ice16,horizontal).
hasOrientation(ice6,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone11,horizontal).
hasOrientation(stone12,horizontal).
hasOrientation(stone14,vertical).
hasOrientation(stone2,vertical).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,horizontal).
hasOrientation(stone7,vertical).
hasOrientation(stone8,vertical).
hasOrientation(stone9,vertical).
hasOrientation(wood13,vertical).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,big).
hasSize(ice15,medium).
hasSize(ice16,small).
hasSize(ice6,big).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,medium).
hasSize(stone11,medium).
hasSize(stone12,medium).
hasSize(stone14,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,small).
hasSize(stone5,small).
hasSize(stone7,medium).
hasSize(stone8,small).
hasSize(stone9,medium).
hasSize(wood13,medium).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct1).
isAnchorPointFor(ice16,struct4).
isAnchorPointFor(pig0,struct3).
isAnchorPointFor(stone11,struct2).
isAnchorPointFor(stone2,struct0).
isBelow(ice10,hill2).
isBelow(ice15,stone14).
isBelow(ice6,stone0).
isBelow(stone0,stone1).
isBelow(stone0,stone3).
isBelow(stone1,stone3).
isBelow(stone11,stone12).
isBelow(stone12,stone14).
isBelow(stone12,wood13).
isBelow(stone3,stone4).
isBelow(stone3,stone5).
isBelow(stone4,stone5).
isBelow(stone5,stone2).
isBelow(stone5,stone7).
isBelow(stone5,stone8).
isBelow(stone5,stone9).
isBelow(stone8,stone7).
isBelow(wood13,stone14).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(stone0,hill2).
isLeft(stone1,hill2).
isLeft(stone11,ice15).
isLeft(stone11,wood13).
isLeft(stone12,ice15).
isLeft(stone2,stone7).
isLeft(stone3,hill2).
isLeft(stone4,hill2).
isLeft(stone5,hill2).
isLeft(stone7,stone8).
isLeft(stone9,hill2).
isOn(ice10,hill0).
isOn(ice15,hill2).
isOn(ice16,ground).
isOn(ice6,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,ice6).
isOn(stone1,hill0).
isOn(stone1,stone0).
isOn(stone11,hill2).
isOn(stone12,stone11).
isOn(stone14,ice15).
isOn(stone14,stone12).
isOn(stone14,wood13).
isOn(stone2,stone5).
isOn(stone3,hill0).
isOn(stone3,stone0).
isOn(stone3,stone1).
isOn(stone4,hill0).
isOn(stone4,stone3).
isOn(stone5,stone3).
isOn(stone5,stone4).
isOn(stone7,stone5).
isOn(stone8,stone5).
isOn(stone8,stone9).
isOn(stone9,stone5).
isOn(wood13,ice15).
isOver(hill0,pig0).
isOver(ice15,pig0).
isOver(ice16,pig0).
isOver(stone14,pig0).
isOver(wood13,pig0).
isRight(ice15,stone11).
isRight(ice15,stone12).
isRight(ice15,wood13).
isRight(stone0,hill1).
isRight(stone1,hill1).
isRight(stone2,hill1).
isRight(stone3,hill1).
isRight(stone4,hill1).
isRight(stone5,hill1).
isRight(stone7,stone2).
isRight(stone9,stone8).
isRight(wood13,stone11).
isRight(wood13,stone12).
isTower(struct0).
isTower(struct1).
pig(pig0,504,342,10,10).
protects(struct2,pig0).
protects(struct4,pig0).
scene_scale(81,0.9935696).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 455.5,330.5,255,[5,51,1.57079633]).
shape(ice15,rect, 500.5,230,489,[10.8139121,45.25280538,2.136283]).
shape(ice16,poly, 518,215,0,[1,[515,211]]).
shape(ice6,rect, 397.5,332.5,357,[7,51,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 401.5,304,161,[6.36465439,25.35959068,0.09424778]).
shape(stone1,rect, 401.5,297.5,174,[6.91445492,25.17137406,0.09424778]).
shape(stone11,rect, 470,241.5,148,[6.14551599,24.18407956,2.6075219]).
shape(stone12,rect, 472.5,230.5,150,[6.16912263,24.34938671,2.41902634]).
shape(stone14,rect, 492,214.5,113,[4.69748625,24.11369259,2.16769893]).
shape(stone2,rect, 392.5,261.5,163,[6.49839219,25.17137406,1.66504411]).
shape(stone3,rect, 402.5,291,178,[7.1175209,25.07726574,0.09424778]).
shape(stone4,rect, 403,284.5,168,[7,24,0]).
shape(stone5,rect, 404,278.5,163,[6.78071713,24.17581209,0.09424778]).
shape(stone7,rect, 400.5,262.5,160,[6.3582819,25.20183029,1.63362818]).
shape(stone8,rect, 407,263,169,[6.96893375,24.2699204,1.66504411]).
shape(stone9,rect, 413.5,263.5,164,[6.53802167,25.17137406,1.66504411]).
shape(wood13,rect, 491,229,135,[5.68296981,23.85230684,2.136283]).
situation_name('situation1_0-7-4-15-0').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
