belongsTo(ice10,struct2).
belongsTo(ice9,struct1).
belongsTo(pig0,struct3).
belongsTo(stone0,struct0).
belongsTo(stone1,struct0).
belongsTo(stone11,struct4).
belongsTo(stone12,struct4).
belongsTo(stone13,struct4).
belongsTo(stone2,struct0).
belongsTo(stone3,struct0).
belongsTo(stone4,struct0).
belongsTo(stone5,struct0).
belongsTo(stone6,struct0).
belongsTo(stone7,struct0).
belongsTo(stone8,struct1).
belongsTo(wood14,struct4).
bird(redbird0).
birdOrder(redbird0,0).
canCollapse(struct0,struct1).
canCollapse(struct1,struct0).
canCollapse(struct2,struct1).
canCollapse(struct2,struct3).
canCollapse(struct3,struct2).
canCollapse(struct3,struct4).
canCollapse(struct4,struct3).
collapsesInDirection(struct0,struct1,away).
collapsesInDirection(struct1,struct0,towards).
collapsesInDirection(struct1,struct2,away).
collapsesInDirection(struct2,struct1,towards).
collapsesInDirection(struct2,struct3,away).
collapsesInDirection(struct3,struct2,towards).
collapsesInDirection(struct3,struct4,away).
collapsesInDirection(struct4,struct3,towards).
ground_plane(384).
hasColor(redbird0,red).
hasForm(ice10,block).
hasForm(ice9,bar).
hasForm(stone0,block).
hasForm(stone1,bar).
hasForm(stone11,block).
hasForm(stone12,block).
hasForm(stone13,block).
hasForm(stone2,block).
hasForm(stone3,block).
hasForm(stone4,block).
hasForm(stone5,block).
hasForm(stone6,block).
hasForm(stone7,block).
hasForm(stone8,bar).
hasForm(wood14,bar).
hasMaterial(ice10,ice,496,220,19,30).
hasMaterial(ice9,ice,455,306,6,50).
hasMaterial(pig0,pork,504,342,10,10).
hasMaterial(stone0,stone,387,337,26,13).
hasMaterial(stone1,stone,389,350,25,6).
hasMaterial(stone11,stone,504,193,20,21).
hasMaterial(stone12,stone,508,179,23,18).
hasMaterial(stone13,stone,518,178,22,18).
hasMaterial(stone2,stone,388,299,16,25).
hasMaterial(stone3,stone,391,331,25,13).
hasMaterial(stone4,stone,392,325,26,13).
hasMaterial(stone5,stone,395,297,18,24).
hasMaterial(stone6,stone,403,321,25,13).
hasMaterial(stone7,stone,404,299,17,25).
hasMaterial(stone8,stone,426,347,25,6).
hasMaterial(wood14,wood,538,195,25,6).
hasOrientation(ice10,vertical).
hasOrientation(ice9,vertical).
hasOrientation(stone0,horizontal).
hasOrientation(stone1,horizontal).
hasOrientation(stone11,vertical).
hasOrientation(stone12,horizontal).
hasOrientation(stone13,horizontal).
hasOrientation(stone2,vertical).
hasOrientation(stone3,horizontal).
hasOrientation(stone4,horizontal).
hasOrientation(stone5,vertical).
hasOrientation(stone6,horizontal).
hasOrientation(stone7,vertical).
hasOrientation(stone8,horizontal).
hasOrientation(wood14,horizontal).
hasSize(hill0,big).
hasSize(hill1,medium).
hasSize(hill2,medium).
hasSize(hill3,medium).
hasSize(ice10,medium).
hasSize(ice9,medium).
hasSize(pig0,medium).
hasSize(redbird0,medium).
hasSize(stone0,medium).
hasSize(stone1,small).
hasSize(stone11,medium).
hasSize(stone12,medium).
hasSize(stone13,medium).
hasSize(stone2,medium).
hasSize(stone3,medium).
hasSize(stone4,medium).
hasSize(stone5,medium).
hasSize(stone6,medium).
hasSize(stone7,medium).
hasSize(stone8,small).
hasSize(wood14,small).
hill(hill0,357,291,200,92).
hill(hill1,358,251,30,59).
hill(hill2,418,251,80,60).
hill(hill3,522,202,31,58).
isAnchorPointFor(ice10,struct2).
isAnchorPointFor(pig0,struct3).
isAnchorPointFor(stone0,struct0).
isAnchorPointFor(stone11,struct4).
isAnchorPointFor(stone8,struct1).
isBelow(ice9,hill2).
isBelow(stone0,stone3).
isBelow(stone0,stone4).
isBelow(stone0,stone6).
isBelow(stone1,stone0).
isBelow(stone11,stone12).
isBelow(stone11,stone13).
isBelow(stone12,stone13).
isBelow(stone2,stone5).
isBelow(stone3,stone4).
isBelow(stone3,stone6).
isBelow(stone4,stone2).
isBelow(stone4,stone5).
isBelow(stone4,stone6).
isBelow(stone4,stone7).
isBelow(stone6,stone2).
isBelow(stone6,stone5).
isBelow(stone6,stone7).
isBelow(stone7,hill2).
isBelow(stone7,stone5).
isBelow(wood14,stone13).
isCollapsable(struct0).
isCollapsable(struct1).
isCollapsable(struct2).
isCollapsable(struct3).
isCollapsable(struct4).
isLeft(stone2,stone7).
isLeft(stone8,ice9).
isOn(ice10,hill2).
isOn(ice9,hill0).
isOn(pig0,hill0).
isOn(stone0,hill0).
isOn(stone0,stone1).
isOn(stone1,hill0).
isOn(stone11,hill3).
isOn(stone12,stone11).
isOn(stone13,stone11).
isOn(stone13,stone12).
isOn(stone13,wood14).
isOn(stone2,hill0).
isOn(stone2,stone4).
isOn(stone2,stone6).
isOn(stone3,hill0).
isOn(stone3,stone0).
isOn(stone4,hill0).
isOn(stone4,stone0).
isOn(stone4,stone3).
isOn(stone5,hill0).
isOn(stone5,stone2).
isOn(stone5,stone4).
isOn(stone5,stone6).
isOn(stone5,stone7).
isOn(stone6,hill0).
isOn(stone6,stone0).
isOn(stone6,stone3).
isOn(stone6,stone4).
isOn(stone7,hill0).
isOn(stone7,stone4).
isOn(stone7,stone6).
isOn(stone8,hill0).
isOn(wood14,hill3).
isOver(hill0,pig0).
isOver(ice10,pig0).
isOver(stone11,pig0).
isOver(stone12,pig0).
isOver(stone13,pig0).
isRight(ice9,stone8).
isRight(stone2,hill1).
isRight(stone7,stone2).
isTower(struct0).
isTower(struct1).
isTower(struct2).
pig(pig0,504,342,10,10).
protects(struct2,pig0).
protects(struct4,pig0).
scene_scale(81,0.99747469).
shape(hill0,poly, 457,338,18400,[12,[357,354],[357,382],[557,383],[556,291],[526,292],[525,353],[463,353],[461,356],[419,353],[417,356],[389,356],[386,353]]).
shape(hill1,poly, 373,281,1770,[4,[358,252],[359,310],[388,309],[387,251]]).
shape(hill2,poly, 458,281,4800,[10,[418,252],[419,310],[449,309],[450,302],[462,303],[463,310],[491,311],[493,307],[498,305],[496,251]]).
shape(hill3,poly, 538,231,1798,[5,[522,230],[524,260],[553,259],[552,202],[523,203]]).
shape(ice10,rect, 505.5,235,186,[6.26279776,29.83587746,2.07345115]).
shape(ice9,rect, 458,331,300,[6,50,1.57079633]).
shape(pig0,ball, 509,346.5,143,[6.75]).
shape(redbird0,ball, 196.5,327.5,78,[5]).
shape(stone0,rect, 400,343.5,163,[6.60938455,24.67945836,0.31415927]).
shape(stone1,rect, 401.5,353,150,[6,25,0]).
shape(stone11,rect, 514,203.5,104,[4.24264069,24.74873734,2.35619449]).
shape(stone12,rect, 519.5,188,117,[4.99942373,23.60031643,0.62831853]).
shape(stone13,rect, 529,187,114,[4.72957523,24.14578158,0.5969026]).
shape(stone2,rect, 396,311.5,150,[6.12071549,24.64303583,1.13097336]).
shape(stone3,rect, 403.5,337.5,174,[7.15671764,24.31868362,0.28274334]).
shape(stone4,rect, 405,331.5,179,[7.18906802,24.9999862,0.28274334]).
shape(stone5,rect, 404,309,164,[6.57125461,25.04482158,1.03672558]).
shape(stone6,rect, 415.5,327.5,159,[6.47541506,24.59767472,0.28274334]).
shape(stone7,rect, 412.5,311.5,123,[4.95478774,24.97259038,1.0681415]).
shape(stone8,rect, 438.5,350,150,[6,25,0]).
shape(wood14,rect, 550.5,198,150,[6,25,0]).
situation_name('situation1_0-7-3-1-2').
slingshotPivot(196.2,331.8).
structure(struct0).
structure(struct1).
structure(struct2).
structure(struct3).
structure(struct4).
